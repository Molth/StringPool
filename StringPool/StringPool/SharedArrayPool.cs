...
namespace System.Buffers
{
    ...
    internal sealed class SharedArrayPool : StringPool
    {
        ...
        /// <summary>
        ///     Allocate a new <see cref="SharedArrayPoolPartitions" /> and try to store it into the <see cref="_buckets" />
        ///     array.
        /// </summary>
        private SharedArrayPoolPartitions CreatePerCorePartitions(int bucketIndex)
        {
            SharedArrayPoolPartitions inst = new SharedArrayPoolPartitions();
        ...
        public override string Rent(int minimumLength)
        {
            ...
            buffer = Utilities.Create(minimumLength);
            if (log.IsEnabled())
            {
                int bufferId = buffer.GetHashCode();
                log.BufferRented(bufferId, buffer.Length, Id, StringPoolEventSource.NoBucketId);
                log.BufferAllocated(bufferId, buffer.Length, Id, StringPoolEventSource.NoBucketId, bucketIndex >= _buckets.Length ? StringPoolEventSource.BufferAllocatedReason.OverMaximumSize : StringPoolEventSource.BufferAllocatedReason.PoolExhausted);
            }

            return buffer;
        }

        public override void Return(string array, bool clearArray = true)
        {
            if (array is null)
            {
            ...
            if (log.IsEnabled() && array.Length != 0)
            {
                ...
                if (!(haveBucket & returned))
                {
                    log.BufferDropped(array.GetHashCode(), array.Length, Id,
                        haveBucket ? bucketIndex : StringPoolEventSource.NoBucketId,
                        haveBucket ? StringPoolEventSource.BufferDroppedReason.Full : StringPoolEventSource.BufferDroppedReason.OverMaximumSize);
                }
            }
        }
        ...
        public bool Trim()
        {
            ...
            if (pressure == Utilities.MemoryPressure.High)
            {
                if (!log.IsEnabled())
                {
                    ...
                    {
                        SharedArrayPoolThreadLocalArray[] array = tlsBuckets.Key;
                ...
                else
                {
                    foreach (KeyValuePair<SharedArrayPoolThreadLocalArray[], object?> tlsBuckets in _allTlsBuckets)
                    {
                        ...
                        {
                            string? buffer = Interlocked.Exchange(ref buckets[i].Array, null);
                            if (buffer != null)
                            {
        ...
        private SharedArrayPoolThreadLocalArray[] InitializeTlsBucketsAndTrimming()
        {
            ...
            SharedArrayPoolThreadLocalArray[] tlsBuckets = new SharedArrayPoolThreadLocalArray[NumBuckets];
    ...
    internal struct SharedArrayPoolThreadLocalArray
    {
        ...
        public SharedArrayPoolThreadLocalArray(string array)
        {
            Array = array;
    ...
    internal sealed class SharedArrayPoolPartitions
    {
        ...
        public SharedArrayPoolPartitions()
        {
            ...
            Partition[] partitions = new Partition[SharedArrayPoolStatics.s_partitionCount];
        ...
        /// <summary>
        ///     Try to push the array into any partition with available space, starting with partition associated with the current
        ///     core.
        ///     If all partitions are full, the array will be dropped.
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public bool TryPush(string array)
        {
        ...
        /// <summary>
        ///     Try to pop an array from any partition with available arrays, starting with partition associated with the current
        ///     core.
        ...
        private sealed class Partition
        {
            ...
            private readonly string?[] _arrays = new string[SharedArrayPoolStatics.s_maxArraysPerPartition];
    ...
    internal static class SharedArrayPoolStatics
    {
        ...
        /// <returns>
        ///     Defaults to 32. This does not factor in or impact the number of arrays cached per thread in TLS (currently
        ///     only 1).
        /// </returns>
